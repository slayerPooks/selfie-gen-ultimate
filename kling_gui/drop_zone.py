...
class DropZone(tk.Frame):
    ...
    def __init__(self, parent, on_files_dropped: Callable[[List[str]], None], **kwargs):
        ...
        self.main_label = tk.Label(
            self.drop_frame,
            text="DRAG IMAGES HERE TO PROCESS",
        ...
        self.sub_label = tk.Label(
            self.drop_frame,
            text="Drop any image files from Windows Explorer\n(.jpg, .png, .webp, .bmp, .gif, .tiff)",
            font=("Segoe UI", 10),
            bg=self._default_bg,
            fg=COLORS["text_dim"]
        )
        ...
        if HAS_DND:
            ...
            self.sub_label.config(text="Drag-drop not available.\nUse the file dialog to add images.")
