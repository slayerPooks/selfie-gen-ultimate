...
def check_video_exists(image_path: str, output_folder: str) -> bool:
    ...
    output_path = get_output_video_path(image_path, output_folder)
...
def get_next_available_path(image_path: str, output_folder: str) -> Path:
    ...
    """
    image_name = Path(image_path).stem
    base_path = Path(output_folder) / f"{image_name}_kling.mp4"
    ...
    counter = 2
    while True:
        new_path = Path(output_folder) / f"{image_name}_kling_{counter}.mp4"
