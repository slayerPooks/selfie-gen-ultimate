...
**Video models** are defined in `models.json` as a list of endpoint strings (not dicts). `model_metadata.py` derives display names from endpoints via `_endpoint_to_short_name()`. The file also has a `user_notes` map for per-model descriptions. At runtime, `ModelSchemaManager` enriches models with pricing and parameter info from the fal.ai API.
...
### Sibling Project & Porting Guide

This is the **advanced version**. A simpler version lives at `F:\claude\kling_ui_complete_2\kling_ui_shareable_20260228_r1` (video-only, no tabs). See `PORTING_GUIDE.md` for which files are shared between versions, structural differences (especially `config_panel.py`), and how to port changes safely. Key: 7 files are identical/copy-as-is, `config_panel.py` is the biggest divergence (inline COLORS vs theme import, health check presence, mini drop zone).

### Known Code Inconsistency

`config_panel.py` duplicates a `COLORS` dict and `FONT_FAMILY` instead of importing from `theme.py`. All other modules use `theme.py`. When editing theme values, update both locations or refactor to use the single source. (This divergence is documented in `PORTING_GUIDE.md` — the simple version already uses the theme import.)
